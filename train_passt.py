...
r"""
===================================================================
PaSST (Patchout faSt Spectrogram Transformer) Transfer Learning
===================================================================
Same recipe as train_panns.py / train_ast.py / train_clap.py: freeze a
pretrained AudioSet model, extract one embedding per clip, train a
small classifier head on top. PaSST is another transformer (like AST)
but with a different training trick (structured "patchout" -- randomly
dropping spectrogram patches during ITS OWN pretraining for speed/
regularization), and is a strong, independently-published AudioSet
competitor to AST. Built as an EXTRA comparison point for the report/
discussion -- NOT expected to beat the traditional ML result (86.47%
test accuracy), same domain-shift reasoning as every other transfer-
learning model tried in this project.
...
WHAT THIS SCRIPT DOES:
  ...
  2) Loads each recording at 32 kHz mono (PaSST's required input rate,
     same as PANNs), trims silence, normalizes, fixes to 5 seconds (via
     the shared audio_common.load_clean_audio, same as every other
     script here).
  3) Runs it through the pretrained PaSST checkpoint (via the
     `hear21passt` package's HEAR-2021 API: `load_model(mode=
     "embed_only")` + `get_scene_embeddings(audio, model)`), which
     already returns one fixed-size (1295-dim) embedding per clip --
     no manual pooling needed, same as CLAP.
  4) Trains a Logistic Regression and an SVM on top of these
     embeddings (same small grid search as the other transfer-learning
     scripts), picks the best on the VALIDATION set, reports final
     metrics on the TEST set.
  Train is NOT augmented (same lesson learned from YAMNet/PANNs/AST/
  CLAP: augmenting a frozen pretrained model's input tends to hurt
  more than help here).

  Like train_clap.py, the classifier head is fit on a class-balanced
  SUBSET of train (not the full 626) -- PaSST is a heavy CPU-only
  transformer on this machine (AST measured ~90-100s per 8-clip batch),
  and a simple Logistic Regression/SVM head doesn't need the full
  train split to fit well. val/test are always full and untouched.
...
REQUIREMENTS (extra, on top of the rest of the project):
    ...
    PaSST checkpoint, cached afterwards under ~/.cache/torch/hub/).

HOW TO RUN:
    cd C:\Users\hp\Desktop\car_test
    python train_passt.py
"""
...
BATCH_SIZE = 8             # PaSST is heavier than PANNs -- smaller batches on CPU

# see the module docstring -- same subset-for-speed rationale as train_clap.py
...
def load_clean_audio_32k(path):
    ...
    normalize/pad + empty-clip fallback logic), just at PaSST's 32kHz rate."""
    y, _ = _shared_load_clean_audio(
        path, target_sr=PASST_SR, target_duration=TARGET_DURATION, fallback_to_untrimmed=True,
    )
    return y.astype(np.float32)


def load_passt_model():
    """Downloads the PaSST checkpoint on first run (cached under
    ~/.cache/torch/hub/ afterwards). CPU by default -- same as the rest of
    this project. mode="embed_only" skips computing the (unused) AudioSet
    classification head, just returns the 1295-dim scene embedding."""
    from hear21passt.base import load_model
    print("   (loading PaSST via hear21passt -- first run downloads the checkpoint)")
...
def extract_embeddings(paths, model, desc="embedding extraction"):
    ...
    with torch.no_grad():
        for i in tqdm(range(0, len(paths), BATCH_SIZE), desc=desc):
            ...
            batch_audio = torch.tensor(np.stack([load_clean_audio_32k(p) for p in batch_paths]))
            emb = get_scene_embeddings(batch_audio, model)
...
def main():
    print("=" * 70)
    print("PaSST (Patchout faSt Spectrogram Transformer) Transfer Learning")
    print("=" * 70)
    ...
    print("(No augmentation on train -- same lesson learned from YAMNet/PANNs/AST/CLAP: augmenting a "
          "frozen pretrained model's input tends to confuse its small classifier head more than help.)")

    print("\n[1] Loading PaSST...")
    passt_model = load_passt_model()

    print("\n[2] Extracting embeddings (scene embeddings, per clip)...")
    X_train = extract_embeddings(list(train_df["matched_path"]), passt_model, "train embeddings")
    X_val = extract_embeddings(list(val_df["matched_path"]), passt_model, "val embeddings")
    X_test = extract_embeddings(list(test_df["matched_path"]), passt_model, "test embeddings")
    ...
    with open(os.path.join(DATA_DIR, "passt_report.txt"), "w", encoding="utf-8") as f:
    ...
    ax.set_title(f"Confusion Matrix (Test) - PaSST + {best_name}")
    plt.tight_layout()
    plt.savefig(os.path.join(DATA_DIR, "passt_confusion_matrix.png"), dpi=150)
    plt.close()

    with open(os.path.join(DATA_DIR, "best_passt_model.pkl"), "wb") as f:
    ...
    np.save(os.path.join(DATA_DIR, "passt_val_probs.npy"), best_model.predict_proba(X_val))
    np.save(os.path.join(DATA_DIR, "passt_test_probs.npy"), best_model.predict_proba(X_test))

    print("\n" + "=" * 70)
    print("DONE. Saved:")
    print("   - best_passt_model.pkl")
    print("   - passt_report.txt")
    print("   - passt_confusion_matrix.png")
    print("   - passt_val_probs.npy / passt_test_probs.npy (for evaluate_ensemble.py)")
    print("=" * 70)
    print("\nCompare this test macro-F1 against traditional_ml_report.txt, cnn_report.txt, "
          "transfer_learning_report.txt (YAMNet), panns_report.txt, ast_report.txt, and "
          "clap_report.txt -- this is a comparison data point for the report/discussion, not "
          "expected to beat the traditional ML result.")
